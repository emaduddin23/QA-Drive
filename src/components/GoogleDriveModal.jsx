...
import { X, HardDrive, Key, Folder, CheckCircle, AlertCircle, RefreshCw, UploadCloud, HelpCircle } from 'lucide-react';
...
export default function GoogleDriveModal({ isOpen, onClose, onSyncComplete }) {
  ...
  const checkDriveStatus = async () => {
    try {
      ...
      setHasCredentials(data.hasCredentials && data.isAuthenticated);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
      <div className="glass-panel w-full max-w-xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-6 max-h-[90vh] overflow-y-auto">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-900 dark:text-white">Google Drive Cloud Connector</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Retrieve QA Knowledge PDFs & Docs via Google Drive API</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
              ...
              <span>Step 1: Google Service Account Credentials</span>
            </label>
            {hasCredentials ? (
          ...
          <div className="space-y-2">
            <textarea
              ...
              placeholder="Paste service account JSON credentials here, or upload credentials.json file below..."
              rows={3}
            ...
            <div className="flex items-center justify-between gap-2">
              <label className="cursor-pointer px-3 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-medium transition flex items-center gap-1.5 border border-slate-200 dark:border-slate-700">
        ...
        <div className="space-y-3 border-t border-slate-200 dark:border-slate-800 pt-4">
          ...
          <div className="space-y-2">
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs font-mono text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-indigo-500"
            />
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-800 pt-4">
          ...
          <button
            ...
            disabled={loading}
            className="px-5 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-600/30 transition flex items-center gap-2 disabled:opacity-50"
          >
            {loading ? (
              <>
