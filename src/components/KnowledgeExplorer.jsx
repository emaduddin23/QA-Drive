...
import { Search, FileText, CheckCircle, Sparkles, BookOpen, Bug, Cpu } from 'lucide-react';

export default function KnowledgeExplorer({ documents, stats, onRefresh }) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 text-xs px-2.5 py-0.5 rounded-full font-semibold border border-indigo-500/20 flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> Indexed Google Drive Repository
              </span>
            </div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">QA Knowledge MCP Database</h2>
          ...
          <div className="flex items-center gap-3">
            <div className="bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-center">
              <div className="text-xl font-bold text-indigo-600 dark:text-indigo-400">{stats?.totalDocs || documents.length}</div>
              <div className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Indexed Files</div>
            </div>
            <div className="bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-center">
              <div className="text-xl font-bold text-emerald-600 dark:text-emerald-400">100%</div>
              <div className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">MCP Sync Rate</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          {/* Document List */}
          <div className="glass-panel rounded-xl p-2 space-y-1 max-h-[500px] overflow-y-auto">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[10px] px-2 py-0.2 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded">
                          {doc.category}
                        </span>
            ...
            <div className="glass-panel rounded-xl p-6 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
                <div>
                  ...
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mt-0.5">{activeDoc.title}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-mono mt-1">{activeDoc.filename}</p>
                </div>
                <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-500/30 px-3 py-1.5 rounded-lg text-emerald-700 dark:text-emerald-300 text-xs font-mono">
                  <CheckCircle className="w-4 h-4" /> Ready for RAG Retrieval
                </div>
              </div>

              {/* Document Content View */}
              <div className="bg-slate-900 text-slate-100 dark:bg-slate-950/90 rounded-xl p-4 border border-slate-800 font-mono text-xs leading-relaxed whitespace-pre-wrap max-h-[440px] overflow-y-auto">
                {activeDoc.content}
              </div>
            </div>
          ) : (
            <div className="glass-panel rounded-xl p-12 text-center text-slate-500 text-xs">
              Select a QA document from the left list to inspect its contents.
            </div>
          )}
        </div>
      </div>
